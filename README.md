...
|---|---|
| 打勾 | 點方塊就標記完成 |
| 拖曳換位置 | 長按方塊任一處（包含圖片）0.18 秒把它「拿起來」，放到別格就換位置；格線外放手會從原格移除並進補做。沒有垃圾桶吸附，避免干擾拖曳 |
| 補做池 | 永遠保留固定高度，沒有項目時顯示淡色空狀態，版面不會上下跳；被換掉／移除但**還沒打勾**的項目會以同款正方形出現並可拖回 |
| 部位進度 | 底部拉盤：11 項與本週自訂訓練的「已排入課表時段數 / 週目標」；排進早／晚格就計數，不必先打勾 |
| 每週模板 | 底部拉盤：把本週存成模板，或用模板重設本週。改模板**套用到往後每一週** |
| 週切換 | `‹ ›` 看前後幾週，每週資料各自獨立 |
| 三個帳號 | 首次開啟選 `bob` / `user1` / `user2`，選過就記住；帳號顯示在「本週」右邊，點它可換人 |
...
快取住反而會讀到舊資料。
每次啟動也會主動檢查新版 service worker；新版接管後自動重新載入一次，避免
iPhone 主畫面 app 長期卡在舊版 JS。
...
兩台同時離線時，上雲走 **read-merge-write**：先讀回雲端現況再合併。
若兩份資料各新增了不同的勾就取聯集，兩邊都保留；若只是單純新增或取消同一批勾，
則採較新的完整狀態。因此 realtime 下取消勾選不會再被舊資料復活。
...
- `domain/week.test.ts` — 週次邊界（跨年 ISO 週）、打勾、替換／拖回補做方塊、舊重複資料修復、進度計算
...
- `App.test.tsx` — 真實點擊流程：打勾、拖曳換位置、格線外進補做、補做正方形拖回、
  建立自訂訓練、排入格子、打勾與確認刪除、週切換、模板存取、
