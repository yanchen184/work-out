...
const rows = [
  ...
  ['ok', '頂部資訊精簡', '移除「每週健身」與同步狀態列，只保留週次導覽，帳號緊接在本週右側',
   '390 × 844 真實畫面確認：第一列依序為上一週、本週、bob、下一週，課表仍完整一屏顯示'],
  ...
  ['ok', '拖曳不再受垃圾桶干擾', '垃圾桶偵測、吸附動畫與畫面已完整移除；格線外放手仍會移入本週補做',
   '元件測試確認往底部中央拖也不出現垃圾桶，放手仍正常從原格消失並進補做'],
  ['ok', '圖片區也能正常拖曳', '圖騰與內層內容不再攔截觸控事件，長按方塊圖片和長按文字使用同一套拖曳流程',
   '390 × 844 真實頁面驗證：圖片中央的命中目標為外層 tile；元件測試從圖片起手後成功浮起方塊'],
  ['ok', '補做正方形可拖回課表', '本週補做使用同款訓練方塊；長按可拖回任一早／晚格，放回後補做項消失',
   '以部位＋原日期＋早晚辨識；回歸測試確認兩個二三頭只移除實際拖回的一顆，× 也只略過指定一顆'],
  ['ok', '補做區固定不跳動', '本週補做即使沒有項目也保留同樣高度，以淡色空狀態穩定整頁版面',
   '390×844 與 320×844 真實畫面確認：七天、空補做區、底部兩按鈕全在一屏，頁面無捲動或橫向溢出'],
  ['ok', '兩顆底部按鈕', '部位進度、每週模板各自從底部彈上來',
   '2026-07-31 真實瀏覽器：兩個拉盤皆正常開闔；進度列圖騰與模板操作卡截圖確認，320px 無橫向溢出'],
  ['ok', '自訂訓練可建立、排入與刪除', '部位進度按 ＋ 建立長跑；主畫面格子的 ＋ 可排入，排入即計入部位進度',
   '完整流程測試：建立 0/1 → 排入週日立即 1/1 → 打勾 → 重開保留；確認刪除會清課表／勾選／補做，永久模板排除'],
...
<div class="wrap">
  ...
  <div class="card">
    <div class="stat">
      ...
      <div><b>163</b><span>單元 + 元件測試</span></div>
  ...
  <div class="card">
    <ul>
      ...
      <li><strong>被頂掉的黏在手上</strong> — 放進已有項目的格子，原本那顆接著被拿在手上，
        可以直接放到別格，或放到格線外移進補做，不用重新拿一次。</li>
      <li><strong>不使用垃圾桶吸附</strong> — 使用者回報拿起後偶爾動不了，因此移除底部偵測區、
        吸附動畫與垃圾桶畫面；所有格線外落點只走一條簡單路徑。iOS 仍從按下就 capture pointer，
        避免 Safari 接管手勢造成「拿得起來但丟不掉」。</li>
      <li><strong>預設模板一天一早一晚</strong> — 早上與晚上各一攤，不是全擠在早上。模板只是起點，隨時可改。</li>
      <li><strong>安排與完成分開看</strong> — 部位進度看「已排入課表 / 週目標」，排進早晚格就算；
        主畫面進度條才看實際打勾完成率。在健身房仍不需要輸入組數。</li>
      <li><strong>自訂訓練有完整生命週期</strong> — 部位進度的 ＋ 建立長跑後先是 0/1，
        再從主畫面格子的 ＋ 排入；之後可打勾、拖曳、進補做與拉回。刪除前確認並清乾淨，
        只屬於本週、不污染永久模板。</li>
      <li><strong>補做池只收「沒打勾」的正方形</strong> — 已經做過的換掉不算欠；補做方塊可直接拖回課表，
        放回後只移除該來源的一顆；同部位不同日期的補做彼此獨立。舊資料若同時留在原格與補做，
        載入時會自動清掉精確原格。補做區永遠保留固定高度，空白時也不讓整頁上下跳。</li>
