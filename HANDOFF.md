...
4. **拖曳仍然能用**。長按 0.18 秒拿起來、放到別格換位置；格線外放手要從原格移除並進補做。
   垃圾桶的偵測、吸附與畫面已依使用者要求完整移除，不可加回。
   「本週補做」區塊即使沒有項目也必須保留同樣高度並顯示空狀態，避免課表與底部按鈕跳動。
   有補做時必須顯示同款正方形，長按可拖回任一早／晚格；同部位有多筆補做時，
   放回或按 × 都只能影響「部位＋原日期＋早晚」完全相同的那一顆。
   同一項若因舊資料同時存在原格與補做，載入時以補做紀錄為準，自動從精確原格移除。
5. **部位進度可建立本週自訂訓練**。標題右側 `＋` 可輸入例如「長跑」，新增後先顯示 `0/1`；
   主畫面任一格按 `＋` 時必須出現在選項中，排入後立即顯示 `1/1`，不必打勾；
   部位進度與其總百分比一律以「已排入課表 / 週目標」計算，主畫面進度條才是打勾完成率。
   排入後可打勾、拖曳、進補做與拉回。
   刪除前要確認，確認後清掉該自訂項目的課表、勾選與補做紀錄；固定 11 項不可刪。
...
6. **主畫面頂部保持乾淨**。不可再顯示「每週健身」與同步狀態文案；
   只顯示週次導覽，使用者名稱緊接在「本週／日期」右邊，點名稱仍可換人。
...
- `user2` 是 production 的同步驗收帳號；2026-07-31 已修正「取消後被舊聯集復活」，
  現在可用正常點擊取消，不需要刪 Firestore 文件。
